    ...
    if( m_DaliBus.GetDLLVersionString( &Str))
     {
       // DLL loaded successfully:
		...
		if( !m_DaliBus.GetDLLVersionString( &Str, GVS_BUSSERVER_VERSION))
...
void CDaliBusAccessTestDlg::OnDestroy() 
{
	...
	m_DaliBus.ClosePort();                 // close the DALI access gateway
...
//		iDevice= DD_AUTODEVICE;
	   ...
	   //
	   HDALI iRet= m_DaliBus.OpenPort( dwPort, iDevice, m_hWnd, &dwSerialNumber);
	   //
	   ...
	   if( iRet > 0)
	   {
		 ...
		 {
			 CString SerNr;
			 SerNr.Format( "  (SerNo.: %d)", dwSerialNumber);
			 Str+= SerNr;
		 }
...
{	
   ...
   BOOL bDTREnable= m_DaliBus.NeedSetDTR( iCmd);
...
void CDaliBusAccessTestDlg::OnBCastOn() 
{
	...
	iRet = m_DaliBus.SendDALIData(CMD_RECALL_MAX);
	SetAnswerText( iRet);
}
...
void CDaliBusAccessTestDlg::OnBCastOff() 
{
	...
	iRet = m_DaliBus.SendDALIData(CMD_OFF);
	SetAnswerText( iRet);
}
...
void CDaliBusAccessTestDlg::OnSendCommand() 
{
	...
	int iCmd= GetDlgItemInt( IDC_CMDNUMBER, NULL, FALSE);
	int iDTR= 0;	
    if( m_DaliBus.NeedSetDTR( iCmd))
		iDTR= GetDlgItemInt( IDC_DTRVAL, NULL, FALSE);

	iRet= m_DaliBus.SendDALIData( iCmd, iDTR);
	SetAnswerText( iRet);
}
